...
if [[ -z "${EXTENSIONS##*,msgpack,*}" ]]; then
    ...
    installExtensionFromTgz msgpack-2.1.0
fi
...
if [[ -z "${EXTENSIONS##*,yac,*}" ]]; then
    ...
    installExtensionFromTgz yac-2.2.1
fi
...
if [[ -z "${EXTENSIONS##*,yar,*}" ]]; then
    ...
    if [[ "$?" = "1" ]]; then
        ...
        installExtensionFromTgz yar-2.1.2
    else
...
if [[ -z "${EXTENSIONS##*,yaconf,*}" ]]; then
    ...
    installExtensionFromTgz yaconf-1.1.0
fi
...
if [[ -z "${EXTENSIONS##*,yaf,*}" ]]; then
    ...
    if [[ "$?" = "1" ]]; then
        ...
        installExtensionFromTgz yaf-3.2.3
    else
